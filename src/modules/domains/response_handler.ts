...
export interface ResponseHandler {
  ...
  translateResponseToDateList(response: AxiosResponse): Result<Date[], ResponseError>;
}

export class ResponseHandlerImpl implements ResponseHandler {
  translateResponseToModel<T>(response: AxiosResponse): Result<T, ResponseError> {
  ...
  translateResponseToDateList(response: AxiosResponse<string[]>): Result<Date[], ResponseError> {
    if (response.status === 200) {
      const result: Date[] = [];
  ...
  private createErrorResponse<T>(response: AxiosResponse): Failure<T, ResponseError> {
